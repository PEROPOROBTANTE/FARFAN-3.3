...
def get_cors_config() -> Dict[str, Any]:
    ...
    logger.info(
        "CORS configured",
        extra={"environment": ENVIRONMENT, "allowed_origins": ALLOWED_ORIGINS},
    )
        ...
        if "Set-Cookie" in response.headers:
            ...
            if "SameSite" not in cookie:
                response.headers["Set-Cookie"] = (
                    f"{cookie}; SameSite=Strict; Secure; HttpOnly"
                )
...
class SecurityAuditLogger:
    ...
    def log_auth_attempt(
        username: str, success: bool, ip_address: str, reason: Optional[str] = None
    ) -> None:
    ...
    def log_suspicious_activity(
        ip_address: str, activity_type: str, details: Dict[str, Any]
    ) -> None:
