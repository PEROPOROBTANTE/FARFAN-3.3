...
class MetricsCollector:
    ...
    def record_cache_hit(self, cache_name: str) -> None:
        ...
        with self._lock:
            ...
            logger.debug(f"Cache hit recorded: {cache_name} (hit rate: {hit_rate:.2f}%)")
    ...
    def record_cache_miss(self, cache_name: str) -> None:
        ...
        with self._lock:
            ...
            logger.debug(f"Cache miss recorded: {cache_name} (hit rate: {hit_rate:.2f}%)")
    ...
    def update_data_freshness(self, data_source: str, timestamp: datetime) -> None:
        ...
        with self._lock:
            ...
            logger.debug(f"Data freshness updated: {data_source} ({age_seconds:.1f}s old)")
    ...
    def record_ws_disconnect(self, connection_id: str, reason: Optional[str] = None) -> None:
        """
            ...
            if connection_id in self._ws_metrics:
                ...
                if self._ws_metrics[connection_id].is_abnormal_disconnect:
                    self._emit_alert(
                        ...
                        context={
                            "connection_id": connection_id,
                            "duration_seconds": self._ws_metrics[connection_id].duration_seconds,
                            "reason": reason
                        }
                    )
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "websockets": {
                    "active": sum(1 for ws in self._ws_metrics.values() if ws.disconnected_at is None),
                    "total": len(self._ws_metrics),
                    "recent_disconnects": len([
                        d for d in self._ws_disconnect_history
    ...
    def _emit_alert(self, alert_type: str, message: str, context: Dict[str, Any]) -> None:
        """
